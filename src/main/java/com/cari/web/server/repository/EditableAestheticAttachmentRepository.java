...
public interface EditableAestheticAttachmentRepository<T extends CreatableTable & EditableAestheticAttachment>
    ...
    default DatabaseUpsertResult<T> createOrUpdateForAesthetic(int pkAesthetic, List<T> objects,
        ...
        Map<Integer, T> payloadToPk = existing.stream()
                .collect(Collectors.toMap(t -> ((T) t).alternateKeyHash(), Function.identity()));
        ...
        return new DatabaseUpsertResult<>(rval, didUpdate);
    }
}
